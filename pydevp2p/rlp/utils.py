...
def format_time(timestamp: bytes) -> str:
    return time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(bytes_to_int(timestamp)))


def bytes_to_typestr(value: bytes, type: str | list | None = None, fieldname=True) -> str:
    ret = ""
    if len(value) == 0:
        ret = f"N/A"
    elif type is None or type == "None":
        # Guessing mode
        ret = f"{bytes_to_hex(value)}" if len(
            value) > 8 else f"{bytes_to_int(value)}"
