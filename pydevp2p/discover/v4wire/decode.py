...
def decodeDiscv4(input: bytes) -> tuple[Header, Packet | None] | None:
    ...
    try:
        ...
        if 6 < bytes_to_int(packetType) < 1:
            print(f"{framectx()} decodeDiscv4(input) Err Invalid Packet Type {bytes_to_int(packetType)}")
            return None    
    ...
    #     return header, None
    # if dec is None:
    #     print(f"{framectx()} decodeDiscv4(input) Err Unable to RLP Decode Message")
    #     return header, None
