...
class Header:
    ...
    def mask(self, destID: bytes):
        mask_key = destID[:16]
        ctr = Counter.new(len(mask_key) * 8,
                          initial_value=bytes_to_int(self.IV))
    ...
    def checkValid(self, packetLen: int) -> bool:
        ...
        if self.protocolID != protocolID:
            print(
                f"StaticHeader checkValid(packetLen) Err Invalid Protocol ID, Expected {protocolID} Got {self.protocolID}")
            return False
        if bytes_to_int(self.version) < minVersion:
            print(
                f"StaticHeader checkValid(packetLen) Err Invalid Protocol Version, Expected {minVersion} Got {self.version}")
            return False
        if bytes_to_int(self.flag) != flagWhoareyou and packetLen < minMessageSize:
            print(
                f"StaticHeader checkValid(packetLen) Err Invalid Msg Len, Expected {packetLen} < {minMessageSize}")
            return False
        if bytes_to_int(self.authSize) > packetLen:
            print(
                f"StaticHeader checkValid(packetLen) Err Invalid Auth Len, Expected {packetLen} < {self.authSize}")
            return False
        return True
...
class WhoareyouAuthData:
    ...
    def __init__(self, authData: bytes) -> None:
        ...
        # uint64 (8 bytes) - Highest known ENR sequence of requester
        self.recordSeq = authData[16:self.SIZE]
...
class Discv5Codec:
    ...
    def decode(self, input: bytes, fromAddr: str) -> tuple[Header | None, Packet | None, Session | None]:
        ...
        if msgFlag == flagWhoareyou:
            ...
            dec_handshake_msg = self.decodeHandshakeMessage(
                fromAddr, head, headerData, msgData)
            ...
            print(
                f"{framectx()} Discv5Codec decode(input, fromAddr) Err Invalid Msg Flag, Expected (0, 1, 2) Got {msgFlag}")
            return None
    ...
    def decodeWhoareyou(self, head: Header, headerData: bytes) -> Packet | None:
        ...
        p = Whoareyou(challengeData=headerData, nonce=head.nonce,
                      idNonce=auth.idNonce, recordSeq=auth.recordSeq)
        return p
    ...
    def decodeHandshakeMessage(self, fromAddr: str, head: Header, headerData: bytes, msgData: bytes) -> tuple[Enode, Packet, Session] | None:
        ...
        msg = self.decryptMessage(
            msgData, head.nonce, headerData, session.readKey)
        if msg is None:
            self.sc.deleteHandshake(auth.srcEnodeId, fromAddr)
    ...
    def decodeHandshake(self, fromAddr: str, head: Header) -> tuple[Enode | None, HandshakeAuthData | None, Session | None, str | None]:
        ...
        # Get node record
        n = self.decodeHandshakeRecord(
            challenge.node, auth.srcEnodeId, auth.record)
        if n is None:
        ...
        session = deriveKeys(self.sha256, self.privk, ephkey,
                             auth.srcEnodeId, self.localEnodeID, cdata)
    ...
    def decodeHandshakeAuthData(self, head: Header) -> HandshakeAuthData | None:
        ...
        if head.authData is None:
            print(
                f"{framectx()} Discv5Codec decodeHandshakeAuthData(head) Err AuthData is None")
            return auth

        vardata = head.authData[HandshakeAuthData.SIZE:]
        sigAndKeySize = bytes_to_int(
            auth.sigSize) + bytes_to_int(auth.pubkSize)
    ...
    def decodeHandshakeRecord(self, local: Enode, wantID: bytes, remote: bytes) -> Enode | None:
        ...
        if len(remote) > 0:
            ...
            if local is None:
                ...
                if n.pubk is None:
                    print(
                        f"{framectx()} Discv5Codec decodeHandshakeRecord(local, wantID, remote) Err Unable to Create ENode")
                    return None
                if n.pubk != wantID:
                    print(
                        f"{framectx()} Discv5Codec decodeHandshakeRecord(local, wantID, remote) Err Found Different ID than Record in Handshake")
                    return None
                node = n
        if node is None:
            print(
                f"{framectx()} Discv5Codec decodeHandshakeRecord(local, wantID, remote) Err No ENR Record Found")
            return None
    ...
    def decodeMessage(self, fromAddr: str, head: Header, headerData: bytes, msgData: bytes) -> Packet | None:
        ...
        if key is None:
            print(
                f"Discv5Codec decodeMessage(fromAddr, head, headerData, msgData) No Known Key for {fromAddr} Initiate Handshake")
    ...
    def decryptMessage(self, input: bytes, nonce: bytes, headerData: bytes, readKey: bytes | None) -> Packet | None:
        ...
        if msgdata is None or len(msgdata) == 0:
            print(
                f"{framectx()} Discv5Codec decryptMessage(input, nonce, headerData, readKey) Err Unable to Decrypt Packet")
            return None
